...
void findCoincidence(TTree* input1,TTree* input2,TTree* input3,TTree* output1,TTree* output2,TTree* output3){
    ...
    input1->SetBranchAddress("Alignment_12",&aligned12_g);
    input1->SetBranchAddress("Alignment_13",&aligned13_g);
    ...
    output1->Branch("event_number_global",&event_number_g_c);
    output1->Branch("file_name",&file_name_c);
    output1->Branch("computer_secIntoEpoch",&computer_secIntoEpoch_g_c);
    output1->Branch("computer_nsIntoSec",&computer_nsIntoSec_g_c);
    output1->Branch("digitizer_duplicate",&digitizer_dup_one_g_c);
    output1->Branch("digitizer_time",&digitizer_time_one_g_c);
    output1->Branch("hit_pmt",&pmtNum_c);
    output1->Branch("hit_charge",&pmtCharge_c);
    output1->Branch("hit_timeStart",&pmtTimeStart_c);
    output1->Branch("hit_timeStop",&pmtTimeStop_c);
    output1->Branch("hit_timePeak",&pmtTimePeak_c);
    output1->Branch("RF_waveform",&RF_ADC_c,"RF_ADC[2100]/s");
    output1->Branch("RF_timeStart",&RF_timeStart_c);
    output1->Branch("RF_timeStop",&RF_timeStop_c);
    output1->Branch("Alignment_12",&aligned12_c);
    output1->Branch("Alignment_13",&aligned13_c);
    ...
    for(int i=0;i<entry1;++i){
        input1->GetEntry(i);
        if(aligned12_g==-1 || aligned13_g==-1)continue;
        if((*RF_timeStart1)[0]==-1)continue;
        if(i%100==0)
        std::cout<<i<<std::endl;
        input2->GetEntry(aligned12_g);
        input3->GetEntry(aligned13_g);
...
#define Coinc
        ...
        for(std::size_t k=0;k<pmtNum1->size();++k){
            pmtNum_c.push_back((*pmtNum1)[k]);
            pmtCharge_c.push_back((*pmtCharge1)[k]);
            pmtTimeStart_c.push_back((*pmtTimeStart1)[k]);
            pmtTimeStop_c.push_back((*pmtTimeStop1)[k]);
            pmtTimePeak_c.push_back((*pmtTimePeak1)[k]);
        }
        for(std::size_t k=0;k<RF_timeStart1->size();++k){
            RF_timeStart_c.push_back((*RF_timeStart1)[k]);
            RF_timeStop_c.push_back((*RF_timeStop1)[k]);
        }
        
        for(int k=0;k<2100;++k){
            RF_ADC_c[k]=RF_ADC1[k];
        }
        
            aligned12_c=aligned12_g;
            aligned13_c=aligned13_g;
